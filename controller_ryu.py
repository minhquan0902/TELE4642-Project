...
class Controller13(app_manager.RyuApp):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.mac_to_port = {}
        self.neigh = defaultdict(
            dict)  
        self.bw = defaultdict(lambda: defaultdict(
            lambda: DEFAULT_BW))  
    ...
    def find_paths_and_costs(self, src, dst):
        ...
        possible_paths = list()
        re = []
        queue = [(0, src, [src])]  # Heap with (cost, node, path)
        ...
        while queue:
            ...
            if node == dst:
                ...
                re.append([path_to_dst, cost_of_path])
                continue

            for neighbor in set(self.neigh[node].keys()) - set(path):
                new_path = path + [neighbor]
                new_cost = self.find_path_cost(new_path)
                heapq.heappush(queue, (new_cost, neighbor, new_path))

        return possible_paths, re
    ...
    def add_ports_to_paths(self, paths, first_port, last_port):
        ...
        count = list(paths_n_ports[0].keys())
        return paths_n_ports,count
    ...
    def topology_discover(self, src, first_port, dst, last_port):
        ...
        plt.clf()
        (paths,re) = self.find_paths_and_costs(src, dst)
        path = self.find_n_optimal_paths(paths)
        (path_with_port, count) = self.add_ports_to_paths(path, first_port, last_port)
        c = 0
        ...
        for i in re:
            if i[0] == count:
                c = i[1]
        self.ax.append(self.t)
        self.ay.append(c)
        line = plt.plot(self.ax,self.ay)
        count_new = [str(x) for x in count]
        if count_new[0] == "5":
            count_new.reverse()
        red_patch = mpatches.Patch(color='red', label=("-".join(count_new)))
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for p in ev.msg.body:
            self.bw[switch_dpid][p.port_no] = (p.tx_bytes - self.prev_bytes[switch_dpid][
                p.port_no]) * 8.0 / 1000000  # Przechowuje zajetosc portu w Mbit/s
